...
function RatingStars({
  ...
  return (
    ...
    >
      ...
      {showValue && (
        <span className="ml-1 text-[10px] sm:text-xs text-gray-500 font-bold uppercase">
          {`(${r.toFixed(1)})`}
        </span>
      )}
    </div>
...
function Alert({ variant = "info", title, children }: AlertProps) {
  ...
  return (
    <div className={cn("rounded-xl border p-3 sm:p-4", styles)}>
      {title && (
        <div className="mb-1 text-sm sm:text-base font-bold uppercase italic">
          {title}
        </div>
      )}
...
function ColorSelector({ colors, selectedColor, onChange }: ColorSelectorProps) {
    ...
    <div className="space-y-2 sm:space-y-3">
      <div className="text-xs sm:text-sm font-black uppercase italic text-gray-900 dark:text-white tracking-wide">
        ...
        {selectedColor ? (
          ...
          <span className="text-gray-400 font-medium normal-case not-italic">
            Choisir
          </span>
        )}
      </div>
      <div className="flex flex-wrap gap-2 sm:gap-3">
        {colors.map((color) => {
          ...
          const tickColor = ["Blanc", "Jaune", "White", "Yellow", "Beige"].includes(
            color
          )
            ? "text-black"
            : "text-white";
            ...
            >
              ...
              {isSelected && (
                <span className="absolute inset-0 grid place-items-center">
                  <CircleCheck
                    className={cn("h-4 w-4 sm:h-5 sm:w-5 drop-shadow-md", tickColor)}
                  />
                </span>
              )}
            </button>
...
function ProductJsonLd({ product }: ProductJsonLdProps) {
...
export default function ProductPageClient({
  ...
  useEffect(() => {
    if (initialProduct) {
      if (initialProduct.colors?.length) setSelectedColor(initialProduct.colors[0]);
      if (initialProduct.taille?.length) setSelectedTaille(initialProduct.taille[0]);
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(
            res.status === 404 ? "Produit non trouvé" : `Erreur serveur (${res.status})`
          );
        }
        ...
        if (p.category) {
          try {
            const rRes = await fetch(
              `${API_URL}/products?category=${encodeURIComponent(p.category)}&limit=8&sort=featured`,
              { cache: "no-store" }
            );
            const rJson = await rRes.json();
            const list: Product[] = Array.isArray(rJson?.data) ? rJson.data : [];
            setRelatedProducts(
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 xl:gap-16">
            ...
            <section className="flex flex-col">
              ...
              <div className="flex items-center gap-2 sm:gap-4 mb-4 sm:mb-6 lg:mb-8">
                ...
                <span className="text-[10px] sm:text-sm font-bold text-gray-500">
                  {reviews.length} AVIS
                </span>
              </div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  {Array.isArray(product.features) && product.features.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                      <h3 className="font-black uppercase italic mb-3 sm:mb-4 text-base sm:text-lg">
                        Points Forts
                      </h3>
                      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                        {product.features.slice(0, 6).map((f, idx) => (
                          <li
                            key={idx}
                            className="flex items-start gap-1.5 sm:gap-2 text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-300"
                          >
                    ...
                    {isReviewFormOpen && (
                      ...
                      >
                        ...
                        {reviewMessage && (
                          <Alert
                            variant={
                              reviewMessage.type === "success" ? "success" : "danger"
                            }
                          >
                            {reviewMessage.text}
                          </Alert>
              ...
              <div className="flex flex-col gap-3 sm:gap-4 mt-8 pt-6 border-t border-gray-100 dark:border-gray-800">
                <div className="flex gap-3 sm:gap-4">
                  ...
                  <div className="flex items-center bg-gray-100 dark:bg-gray-800 rounded-xl sm:rounded-2xl p-0.5 sm:p-1">
                    ...
                    <span className="w-10 sm:w-12 text-center font-black text-base sm:text-lg">
                      {quantity}
                    </span>
                    <button
                      onClick={() => handleQuantityChange(quantity + 1)}

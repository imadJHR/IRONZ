...
import React, { useState, useEffect, useMemo, useCallback, useDeferredValue, use } from "react";
...
const ProductCard = React.memo(function ProductCard({ product, viewMode = "grid", handleAddToCart }) {
...
export default function ProductsPage({ initialProducts = [] }) {
  ...
  const [totalInventory, setTotalInventory] = useState(initialProducts.length);
  ...
  useEffect(() => {
    // Skip fetching if we already have initial products from SSR
    if (initialProducts.length > 0) {
      // Initialize price range from initial products

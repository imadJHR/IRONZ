...
export default function EditProductPage() {
    ...
    const fetchProduct = async (): Promise<void> => {
      try {
        ...
        if (!result.success && !result.data) {
          if (result._id) {
            (result as ProductApiResponse).data = result as unknown as ProductData;
          } else {
            throw new Error('Produit introuvable');
          }
        }

        const product: ProductData = result.data || (result as unknown as ProductData);
        ...
        setFormData({
          ...
          shippingWeight: String(product.shipping?.weight || ''),
  ...
  const handleMainImageUpload = (e: ChangeEvent<HTMLInputElement>): void => {
  ...
  const handleGalleryUpload = (e: ChangeEvent<HTMLInputElement>): void => {
  ...
  const validateForm = (): boolean => {
    if (!formData.name.trim()) {
      showNotification('Nom du produit requis', 'error');
      return false;
    }
  ...
  const handleReviewSubmit = async (
    ...
    try {
      ...
      if (contentType && contentType.indexOf('application/json') !== -1) {
        ...
        throw new Error(
          res.status === 500 ? 'Erreur serveur interne (500)' : text
        );
      }

      if (!res.ok) {
        throw new Error(data?.message || "Erreur lors de la sauvegarde de l'avis");
      }

      showNotification(
        isEditing ? 'Avis modifié avec succès' : 'Avis ajouté avec succès'
      );
      resetReviewForm();
      fetchReviews();
    } catch (error) {
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (formData.rating) formDataToSend.append('rating', String(formData.rating));
  ...
  const completion =
    formData.name && formData.description && formData.price ? '100%' : '50%';
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <div className="flex items-center gap-3 px-4 py-3 bg-white rounded-2xl border border-gray-200 shadow-sm">
                <div className="p-2 rounded-lg bg-gray-100">
                  {selectedCategory && (
                    <selectedCategory.icon className="w-5 h-5 text-gray-900" />
                  )}
                </div>
                <div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100">
                  ...
                  <span
                    className={`font-bold ${
                      mainImagePreview ? 'text-green-600' : 'text-red-500'
                    }`}
                  >
                    ...
                    {formData.discount &&
                      parseFloat(formData.discount) > 0 && (
                        <div className="text-sm text-gray-500 line-through">
                          {formatCurrency(
                            parseFloat(formData.oldPrice || formData.price)
                          )}
                        </div>
                      )}
                  </div>
              ...
              <div className="p-6">
                <form onSubmit={handleSubmit} className="space-y-8">
                          ...
                          {features.map((feature, index) => (
                            ...
                            >
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  handleRemoveItem(features, setFeatures, index)
                                }
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {(
                      [
                        { name: 'width', label: 'Largeur', placeholder: 'cm' },
                        { name: 'height', label: 'Hauteur', placeholder: 'cm' },
                        { name: 'depth', label: 'Profondeur', placeholder: 'cm' },
                        { name: 'weight', label: 'Poids', placeholder: 'kg' },
                      ] as { name: keyof FormData; label: string; placeholder: string }[]
                      ...
                      <div key={name} className="space-y-1">
                        ...
                        <input
                          ...
                          className="w-full p-3 border border-gray-300 text-black rounded-xl focus:border-yellow-500 outline-none"
                        />
                      </div>
                    ))}
                  </div>
                  ...
                  {/* Materials */}
                  <div className="mt-4">
                    <label className="text-sm text-black font-bold block mb-2">
                      Matériaux
                    </label>
                    <div className="flex gap-2 text-black">
                      <input
                        ...
                        className="flex-1 p-3 border border-gray-300 rounded-xl focus:border-yellow-500 outline-none"
                      />
                      <button
                        type="button"
                        onClick={() =>
                        ...
                        className="bg-black text-white px-4 rounded-xl font-bold hover:bg-gray-800 transition-all"
                      >
                        +
                      </button>
                    </div>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {materials.map((m, i) => (
                        <span
                          key={i}
                          className="bg-gray-100 text-gray-900 px-3 py-1 rounded-full text-sm cursor-pointer hover:bg-red-100 hover:text-red-600 transition-colors"
                          onClick={() =>
                            handleRemoveItem(materials, setMaterials, i)
                          }
                        >
                          {m} ×
                        </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Colors */}
                    <div>
                      <label className="font-bold text-black text-sm block mb-2">
                        Couleurs
                      </label>
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 p-3 border border-gray-300 text-black rounded-xl focus:border-yellow-500 outline-none"
                          placeholder="Ajouter couleur"
                        />
                        <button
                          type="button"
                          onClick={() =>
                            handleAddItem(colors, setColors, newColor, setNewColor)
                          }
                          className="bg-black text-white px-4 rounded-xl hover:bg-gray-800 transition-all"
                        >
                          +
                        </button>
                      </div>
                      <div className="flex flex-wrap gap-2 mt-2">
                        {colors.map((c, i) => (
                          <span
                            ...
                            className="bg-gray-100 text-gray-900 px-3 py-1 rounded-full text-sm cursor-pointer hover:bg-red-100 hover:text-red-600 transition-colors"
                            onClick={() =>
                              handleRemoveItem(colors, setColors, i)
                            }
                          >
                            {c} ×
                          </span>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 p-3 border border-gray-300 text-black rounded-xl focus:border-yellow-500 outline-none"
                          placeholder="Ajouter taille"
                        />
                        <button
                          type="button"
                          onClick={() =>
                            handleAddItem(taille, setTaille, newTaille, setNewTaille)
                          }
                          className="bg-black text-white px-4 rounded-xl hover:bg-gray-800 transition-all"
                        >
                          +
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {taille.map((t, i) => (
                          <span
                            ...
                            className="bg-gray-100 text-gray-900 px-3 py-1 rounded-full text-sm cursor-pointer hover:bg-red-100 hover:text-red-600 transition-colors"
                            onClick={() =>
                              handleRemoveItem(taille, setTaille, i)
                            }
                          >
                            {t} ×
                          </span>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onKeyPress={(e) =>
                          handleKeyPress(e, tags, setTags, newTag, setNewTag)
                        }
                        placeholder="Ajouter un tag..."
                        className="flex-1 px-4 py-3 rounded-xl border border-gray-300 focus:border-yellow-500 outline-none"
                      />
                      <button
                        type="button"
                        onClick={() =>
                          handleAddItem(tags, setTags, newTag, setNewTag)
                        }
                        className="px-6 py-3 rounded-xl bg-black text-white font-bold hover:bg-gray-800 hover:shadow-lg"
                      >
                        <Plus className="w-5 h-5" />
                      </button>
                    </div>
                    {tags.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-3">
                        {tags.map((tag, index) => (
                          <span
                            key={index}
                            className="bg-gray-100 text-gray-900 px-3 py-1 rounded-full font-bold text-sm cursor-pointer hover:bg-red-100 hover:text-red-600 transition-colors"
                            onClick={() =>
                              handleRemoveItem(tags, setTags, index)
                            }
                          >
                            #{tag}
                          </span>
                    ...
                    <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                      {reviews.length === 0 ? (
                        ...
                        reviews.map((review) => (
                          <div
                            key={review._id}
                            className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex flex-col md:flex-row gap-4 items-start md:items-center justify-between"
                          >
                            <div className="flex-1">
                  ...
                  <div className="pt-8 border-t border-gray-200 flex justify-end gap-4">
                    <button
                      type="button"
                      onClick={() => router.back()}
                      className="px-8 py-4 rounded-2xl border-2 border-gray-300 text-gray-700 font-bold hover:bg-gray-50"
                    >
                      Annuler
                    </button>
                    <button
                      type="submit"
                      disabled={isSubmitting}
                      className="group relative px-12 py-4 bg-yellow-500 hover:bg-yellow-400 text-black font-bold rounded-2xl shadow-lg hover:shadow-xl disabled:opacity-50"
                    >

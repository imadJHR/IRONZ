...
export default function ProductListPage() {
    ...
    try {
      ...
      while (hasMore) {
        ...
        if (data.success && Array.isArray(data.data)) {
          const newItems: Product[] = data.data;
  ...
  const totalInventory: number = products.length;
  const totalPages: number = Math.ceil(totalItems / itemsPerPage);
  const startIndex: number = (currentPage - 1) * itemsPerPage;
  const endIndex: number = Math.min(startIndex + itemsPerPage, totalItems);
  const paginatedProducts: Product[] = filteredProducts.slice(startIndex, startIndex + itemsPerPage);

  const formatPrice = (price: number): string =>
    new Intl.NumberFormat('fr-MA', { style: 'currency', currency: 'MAD' }).format(price);
              ...
              <div className="flex items-center gap-2 mt-1">
                {loading ? (
                  ...
                  <p className="text-gray-500">
                    Inventaire total :{' '}
                    <span className="font-bold text-yellow-600">{totalInventory}</span> articles
                  </p>
                )}
                <button

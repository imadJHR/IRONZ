...
export default function CategoryPage() {
  ...
  const onAddToCart = (product: Product) => {
    addToCart({ ...product, id: (product._id || product.id) as string, quantity: 1 });
  };
      ...
      <div className="container mx-auto px-4">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            ...
            <div className="w-full lg:w-auto flex flex-col sm:flex-row gap-3">
                ...
                <SheetContent side="left" className="w-[85vw] sm:w-[400px] overflow-y-auto bg-white dark:bg-gray-900 z-[100] p-6">
                  ...
                  <div className="py-4 space-y-8">
                    ...
                    <div>
                      ...
                      <div className="px-2">
                        <Slider defaultValue={[0, 20000]} min={0} max={20000} step={100} value={priceRange} onValueChange={handlePriceChange} className="mb-6" />
              ...
              <div className="relative w-full sm:flex-1 lg:w-[320px]">
                ...
                />
                {searchQuery && <X className="absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 cursor-pointer" onClick={() => setSearchQuery("")} />}
              </div>
            </div>
            ...
            <div className="w-full lg:w-auto flex items-center justify-between lg:justify-end gap-3">
              ...
              <div className="hidden sm:flex items-center bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
                <button onClick={() => setViewMode("grid")} className={cn("p-2 rounded-lg", viewMode === "grid" ? "bg-white dark:bg-gray-700 text-yellow-500 shadow-sm" : "text-gray-500")}>
                  <Package className="w-5 h-5" />
                </button>
                <button onClick={() => setViewMode("list")} className={cn("p-2 rounded-lg", viewMode === "list" ? "bg-white dark:bg-gray-700 text-yellow-500 shadow-sm" : "text-gray-500")}>
        ...
        <AnimatePresence>
          {(priceRange[0] > 0 || priceRange[1] < 20000 || searchQuery || selectedSubCategories.length > 0) && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-wrap items-center gap-2 mb-8 p-4 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800">
              <span className="text-sm font-black uppercase text-gray-500">Filtres:</span>
              {searchQuery && <Badge className="gap-2 rounded-xl">"{searchQuery}"<X className="h-3 w-3 cursor-pointer" onClick={() => setSearchQuery("")} /></Badge>}
              {selectedSubCategories.map(sub => (
                <Badge key={sub} className="gap-2 rounded-xl">{sub}<X className="h-3 w-3 cursor-pointer" onClick={() => handleSubCategoryToggle(sub)} /></Badge>
              ))}
              {(priceRange[0] > 0 || priceRange[1] < 20000) && (
                <Badge className="gap-2 rounded-xl">Prix: {formatPrice(priceRange[0])} - {formatPrice(priceRange[1])}<X className="h-3 w-3 cursor-pointer" onClick={() => setPriceRange([0, 20000])} /></Badge>
              )}
              <Button variant="ghost" size="sm" onClick={clearFilters} className="text-red-500 text-xs">Tout effacer</Button>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          <div className="hidden lg:block w-80 flex-shrink-0">
            <div className="sticky top-32 space-y-8">
              <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 p-6">
                <h3 className="text-xl font-black uppercase italic mb-6"><Filter className="inline h-5 w-5 mr-2" />Filtres</h3>
                {availableSubCategories.length > 0 && (
                  <div className="mb-8">
                    ...
                    <div className="space-y-3 max-h-72 overflow-y-auto pr-2 custom-scrollbar">
                      {availableSubCategories.map((sub) => (
                        <div key={sub} className="flex items-center space-x-3">
                          <Checkbox id={`desktop-sub-${sub}`} checked={selectedSubCategories.includes(sub)} onCheckedChange={() => handleSubCategoryToggle(sub)} />
                          <Label htmlFor={`desktop-sub-${sub}`} className="text-sm font-medium cursor-pointer hover:text-yellow-500 transition-colors">{sub}</Label>
                        </div>
                      ))}
                    </div>
                    <Separator className="my-6" />
                ...
                <div>
                  ...
                  <Slider defaultValue={[0, 20000]} min={0} max={20000} step={100} value={priceRange} onValueChange={handlePriceChange} className="mb-8" />
          ...
          <div className="flex-1">
            {filteredProducts.length === 0 ? (
              <div className="text-center py-20 bg-white dark:bg-gray-900 rounded-3xl border-2 border-dashed border-gray-200">
                ...
                <Button onClick={clearFilters} variant="outline" className="rounded-xl">Réinitialiser les filtres</Button>
              </div>
            ) : (
              <motion.div
                className={cn(
              ...
              >
                {filteredProducts.map((product) => {
                  ...
                  return (
                    ...
                    >
                      <div className={cn("relative overflow-hidden bg-gray-50", viewMode === "grid" ? "h-64 sm:h-72" : "w-full sm:w-64 h-64 sm:h-auto flex-shrink-0")}>
                        <Link href={`/produit/${product.slug || id}`}>
                          <CloudImg fill src={product.image} alt={product.name} className="object-cover transition-transform duration-700 group-hover:scale-110" />
                        </Link>
                        <div className="absolute top-4 left-4 flex flex-col gap-2">
                          {product.isNewProduct && <span className="bg-green-500 text-white text-xs font-black uppercase px-2 py-1 rounded-lg">Nouveau</span>}
                          {Number(product.discount) > 0 && <span className="bg-red-500 text-white text-xs font-black uppercase px-2 py-1 rounded-lg">-{product.discount}%</span>}
                        </div>
                        <button onClick={() => toggleFavorite(product)} className={cn("absolute top-4 right-4 p-2 rounded-full bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm transition-all", isInFavorites(id) ? "text-red-500" : "text-gray-400")}>
                          <Heart className={cn("w-5 h-5", isInFavorites(id) && "fill-current")} />
                        </button>
                      </div>

                      <div className="p-5 md:p-6 flex flex-col flex-1">
                        <div className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-2">{product.subCategory || product.category}</div>
                        <Link href={`/produit/${product.slug || id}`} className="mb-3">
                          <h3 className="font-black text-lg md:text-xl text-gray-900 dark:text-white line-clamp-2 min-h-[3rem] group-hover:text-yellow-500 transition">{product.name}</h3>
                        </Link>
                        <div className="mb-4">{renderRating(product.rating)}</div>
                        <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
                          <div>
                            <div className="font-black text-xl text-gray-900 dark:text-white">{formatPrice(product.price)}</div>
                            {product.oldPrice > 0 && <div className="text-xs text-gray-400 line-through">{formatPrice(product.oldPrice)}</div>}
                          </div>
                          <Button size="icon" className="rounded-full bg-yellow-500 text-black shadow-lg" onClick={() => onAddToCart(product)}>
            ...
            <div className="mt-12 pt-8 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
              <p className="text-gray-500 text-sm font-medium">Affichage de <span className="font-black text-gray-900 dark:text-white">{filteredProducts.length}</span> produits</p>
              <Button variant="outline" onClick={clearFilters} className="rounded-xl w-full sm:w-auto">Réinitialiser les filtres</Button>
            </div>
          </div>
        </div>
      </div>

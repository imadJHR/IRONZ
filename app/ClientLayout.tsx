...
import React, { useState, useEffect, useRef, useCallback, useMemo, MouseEvent, FormEvent, ChangeEvent } from "react";
...
const ConditionalSignedOut: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  if (!hasClerkKeys) return null;
  return <SignedOut>{children}</SignedOut>;
};

const ConditionalSignedIn: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  if (!hasClerkKeys) return null;
  return <SignedIn>{children}</SignedIn>;
};

const ConditionalUserButton: React.FC<React.ComponentProps<typeof UserButton>> = (props) => {
  if (!hasClerkKeys) return null;
  return <UserButton {...props} />;
};

const ConditionalSignInButton: React.FC<ConditionalButtonProps> = ({ children, ...props }) => {
  if (!hasClerkKeys) return null;
  return <SignInButton {...props}>{children}</SignInButton>;
};

const ConditionalSignUpButton: React.FC<ConditionalButtonProps> = ({ children, ...props }) => {
  if (!hasClerkKeys) return null;
...
export default function ClientLayout({ children }: ClientLayoutProps) {
  ...
  const toggleLanguage = useCallback(() => {
    setLanguage((prevLanguage: Language) => (prevLanguage === "fr" ? "en" : "fr"));
  }, []);
...
// Navbar Component
const Navbar = React.memo(function Navbar({ language, toggleLanguage }: NavbarProps) {
  ...
  const [favoritesDropdownOpen, setFavoritesDropdownOpen] = useState<boolean>(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent | globalThis.MouseEvent) => {
      ...
      if (
        ...
        !favoritesRef.current.contains(target)
      ) {

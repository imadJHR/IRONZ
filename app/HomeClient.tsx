...
import { useState, useEffect, useMemo, useRef, MouseEvent, ChangeEvent, CSSProperties } from "react";
...
function CloudImg({
  ...
  return (
    <Image
      ...
      sizes={sizes}
      style={{ objectFit: isFill ? "cover" : "contain", ...props.style } as CSSProperties}
      {...(props as Omit<ImageProps, "src" | "alt" | "fill" | "width" | "height" | "className" | "sizes" | "style" | "onError" | "loading" | "priority">)}
    />
  );
}
...
function renderRating(rating: number | string | null | undefined): React.ReactNode {
...
/* -----------------------------
   SWIPER NAV BUTTONS
------------------------------ */
function SwiperNavButtons({ prevRef, nextRef, className = "" }: SwiperNavButtonsProps) {
  return (
...
------------------------------ */
function ProductCard({ product, addToCart, toggleFavorite, isFavorite }: ProductCardProps) {
...
------------------------------ */
function ProductCardLarge({ product, addToCart, toggleFavorite, isFavorite }: ProductCardProps) {
  const productId = product._id || product.id || "";
...
------------------------------ */
function ProductCardModern({ product, addToCart, toggleFavorite, isFavorite, index }: ProductCardModernProps) {
  const productId = product._id || product.id || "";
...
function LatestProductsSection({
  ...
  const uniqueCategories = useMemo(() => {
    const cats = products.reduce((acc: string[], product) => {
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="flex-1 max-w-md">
              <div className="flex flex-wrap gap-2">
                ...
                {uniqueCategories.filter(cat => cat !== 'all').slice(0, 4).map(category => (
                  <button
                    key={category}
          ...
          <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-700">
            ...
            <div className="relative pt-1">
              <input
                ...
                step={100}
                value={priceRange[0]}
                onChange={(e: ChangeEvent<HTMLInputElement>) => setPriceRange([Number(e.target.value), priceRange[1]])}
              ...
              <input
                ...
                step={100}
                value={priceRange[1]}
                onChange={(e: ChangeEvent<HTMLInputElement>) => setPriceRange([priceRange[0], Number(e.target.value)])}
                className="absolute w-full h-2 bg-transparent appearance-none pointer-events-auto [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-yellow-500 [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-white [&::-webkit-slider-thumb]:shadow-lg"
              />
...
export default function HomeClient({
  ...
  const [vedetteProducts, setVedetteProducts] = useState<Product[]>(initialVedetteProducts);
  const [loading, setLoading] = useState<boolean>(false);
  const [showPopup, setShowPopup] = useState<boolean>(false);

  // Latest products pagination
  const [latestProducts, setLatestProducts] = useState<Product[]>(initialLatestProducts);
  const [latestPage, setLatestPage] = useState<number>(1);
  const [hasMoreLatest, setHasMoreLatest] = useState<boolean>(initialLatestProducts.length === 12);
  const [loadingLatest, setLoadingLatest] = useState<boolean>(false);
  const [latestTotal, setLatestTotal] = useState<number>(initialLatestTotal);
  ...
  const loadLatestProducts = async (page: number = 1) => {
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-gray-950 overflow-x-hidden">
          ...
          <div className="container mx-auto px-4 sm:px-6 relative z-10">
            ...
            >
              {discountedProducts.map((product) => (
                <SwiperSlide key={product._id || product.id || Math.random()} className="h-auto">
                  <ProductCard
                    product={product}
                  ...
                  <div className="absolute inset-0 transform -skew-x-6 border-r-4 border-b-4 border-transparent hover:border-yellow-500 transition-all duration-300 bg-neutral-800 overflow-hidden group-hover:-translate-y-2 group-hover:shadow-[10px_10px_0px_0px_rgba(234,179,8,0.2)]">
                    ...
                    <div className="absolute -top-4 -right-2 z-0">
                      <span className="text-9xl font-black italic text-transparent opacity-20 group-hover:opacity-10 transition-opacity duration-300"
                        style={{ WebkitTextStroke: '2px #fff' } as CSSProperties}>
                        0{i + 1}
                      </span>
                    </div>
      ...
      {featuredVedette && (
        <section className="py-20 sm:py-28 lg:py-32 bg-gray-950 relative overflow-hidden flex items-center min-h-[800px]">
              ...
              <div className="w-full lg:w-1/2 flex flex-col justify-center">
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row gap-5 mb-14">
                    <button
                      onClick={(e: MouseEvent<HTMLButtonElement>) => { e.preventDefault(); handleAddToCart(featuredVedette); }}
          ...
          <div className="absolute top-1/2 left-0 w-full overflow-hidden pointer-events-none select-none z-0 opacity-[0.02] -translate-y-1/2 flex flex-col items-center justify-center pointer-events-none mix-blend-overlay">
            ...
            <h2 className="text-[20vw] font-black uppercase italic whitespace-nowrap text-transparent text-center leading-none tracking-tighter" style={{ WebkitTextStroke: '2px white' } as CSSProperties}>
              PRO GEAR
            </h2>
          </div>
        </section>
      ...
      >
        <Image src={logo} alt="logo" className="w-6 h-6 sm:w-8 sm:h-8" width={32} height={32} />
      </motion.button>
